 ...
 */
package br.gov.pr.londrina.test.rest.rest;
...
import br.gov.pr.londrina.test.rest.data.MemberRepository;
...
public class MemberResourceRESTService {
	...
	public List<PessoaFisica> listAllMembers() {
		return repository.obterTodasAsPessoas();
	}

	@GET
	@Path("/{cpf:[0-9][0-9]*}")
	@Produces(value = { "application/json; charset=\"UTF-8\"", "application/xml; charset=\"UTF-8\"" })
	public PessoaFisica obterPessoaPorCpf(@PathParam("cpf") Integer cpf) {
		PessoaFisica pf = repository.findByCpf(cpf);
		if (pf == null) {
	...
	// on the type of error. If the error is standard
	// * bean validation errors then it will throw a ConstraintValidationException
	// with the set of the constraints violated.
	// * </p>
	// * <p>
	// * If the error is caused because an existing member with the same email is
	// registered it throws a regular validation
	...
	// fields, and their message. This can then be used
